    ...
    (let ((item-height (generic-tree-pane-item-height pane))
      ...
      (destructuring-bind (value &rest children) node
        (multiple-value-bind (background foreground)
          ...
          (let ((arrow-width 5)
                (arrow-height 5))
            (draw-right-arrow pane (make-bounding-rectangle x0 y0
                                                            (+ x0 arrow-width) (+ y0 arrow-height)))
...
(defun generic-tree-pane-select-item (pane item-value)
...
Returns :select or :deselect, depending on what action was performed."
  (if (tree-pane-exclusive-p pane)
      (progn
        ...
        :select)
      (let ((member (member item-value (gadget-value pane) :test (tree-pane-test pane))))
...
(defun generic-tree-pane-add-selected-items (pane item-values)
...
(defun generic-tree-pane-deselect-items (pane item-values)
  ...
  (with-bounding-rectangle* (sx0 sy0 sx1 sy1)  (sheet-region pane)
    (declare (ignorable sx0 sx1 sy1))
      (with-slots (items) pane
        (let* ((item-height (generic-tree-pane-item-height pane))
               (number-of-items (generic-tree-pane-items-length pane))
               (n (floor (- my sy0) item-height))
               (index (and (>= n 0)
                           (< n number-of-items)
                           n))
               (item-value (and index (elt (generic-tree-pane-item-values pane) index))))
              ...
              ((not (zerop (logand modifier +shift-key+)))
               (if (and (numberp last-index)
                   ...
                   (funcall (if (eql last-action :select)
                            ...
                            pane
                            (coerce (subseq (generic-tree-pane-item-values pane)
